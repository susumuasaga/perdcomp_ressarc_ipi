...
export default async function writeR15(
  ...
  const notaFiscalDocs = await notaFiscalModel.find()
                          ...
                          .where('codSituacao').equals(1)
                          .where('ipiCredOuDeb').gt(0)
                          .sort({ emitenteCNPJ: 1, num: 1, serie: 1 });
  const out = fs.createWriteStream(`R15${formatAAAA(paAno)}${formatMM(paMes)}.txt`);
  ...
  for (let i = 0; i < notaFiscalDocs.length; i++) {
    ...
    await writeAsync('1'); //14
    await writeAsync('0'); //15
    await writeAsync(formatMM(d.paMes)); //16
    await writeAsync(formatAAAA(d.paAno)); //17
    await writeAsync('\r\n'); //18
  }
  await endAsync();
